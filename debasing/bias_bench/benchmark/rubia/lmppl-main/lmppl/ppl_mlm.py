    ...
    def get_perplexity(self, input_texts: str or List, batch: int = None):
        ...
        for x in input_texts:
            ...
            def encode_mask(mask_position: int):
                ...
                _x = self.sp_token_prefix + _x + self.sp_token_suffix
                ...
                if self.max_length is None:
                  ...
                  _x = _x + [self.tokenizer.pad_token] * (max_length - len(_x))
                _x = torch.IntTensor([self.tokenizer.convert_tokens_to_ids(_x)])
                token_type_ids = torch.IntTensor([0] * _x.shape[1]).unsqueeze(0)
                attention_mask = torch.IntTensor([1] * _x.shape[1]).unsqueeze(0)
                _e = {"input_ids": _x,
