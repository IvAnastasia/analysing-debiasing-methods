...
def compute_nationality_subspace(data, model, tokenizer, batch_size=32):
    """Returns nationality subspace components for SentenceDebias.
    ...
    n_batches = len(data) // batch_size
    for i in tqdm(range(n_batches), desc="Encoding nationality examples"):
        offset = batch_size * i

        inputs_r1 = tokenizer(
            [example["nationality_example_bias"] for example in data[offset : offset + batch_size]],
            return_tensors="pt",
            truncation=True,
            padding="max_length",
            max_length=128,
        ...
        inputs_r2 = tokenizer(
            [example["nationality_example"] for example in data[offset : offset + batch_size]],
            return_tensors="pt",
            truncation=True,
            padding="max_length",
            max_length=128,
    ...
    means = (all_embeddings_r1 + all_embeddings_r2) / 2.0
    all_embeddings_r1 -= means
    all_embeddings_r2 -= means

    all_embeddings = np.concatenate(
        [all_embeddings_r1, all_embeddings_r2], axis=0
    )
