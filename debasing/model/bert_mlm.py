...
class BertForMaskedLM(BertPreTrainedModel):
	...
	def forward(
		...
		labels: Optional[torch.Tensor] = None, # (bsz,seq_len) see below
		...
		prediction_scores = self.cls(sequence_output) # (bsz,seq_len,vocab_size)
		...
		if labels is not None:
			...
			# ((bsz*seq_len,vocab_size),(bsz*seq_len))
		...
		return MaskedLMOutput(
			loss=masked_lm_loss, # (1)
			logits=prediction_scores, # (bsz,seq_len,vocab_size)
...
class BertPrefixTuningForMaskedLM(BertPreTrainedModel):
	...
	def forward(
		...
		if attention_mask==None: # assume there are no paddings in the input
			attention_mask = torch.ones(input_ids.shape).to(self.bert.device) # (bsz,seq_len)
...
class BertPromptTuningForMaskedLM(BertPreTrainedModel):
	...
	def forward(
		...
		raw_embedding = self.bert.embeddings.word_embeddings(input_ids) # (bsz,seq_len,hidden_size)

		prompts = self.get_prompt(batch_size=batch_size) # (bsz,prefix_len,hidden_size)
		inputs_embeds = torch.cat((prompts, raw_embedding), dim=1) # (bsz,prefix_len+seq_len,hidden_size)
		...
		# sequence_output = outputs[0] # (bsz,seq_len,hidden_size)
		sequence_output = outputs[0] # (bsz,prefix_len+seq_len,hidden_size)
		sequence_output = sequence_output[:, self.pre_seq_len:, :].contiguous() # (bsz,seq_len,hidden_size)
		#########################################################################################################
		prediction_scores = self.cls(sequence_output) # (bsz,seq_len,vocab_size)
...
class BertPromptingForMaskedLM(BertPreTrainedModel):
	'''
	...
	def forward(
		...
		prompts = self.get_prompt(batch_size=batch_size) # (bsz,pre_seq_len,hidden_size)
		inputs_embeds = torch.cat((prompts, raw_embedding), dim=1) # (bsz,pre_seq_len+seq_len,hidden_size)
		prefix_attention_mask = torch.ones(batch_size, self.pre_seq_len).to(self.bert.device)
		if attention_mask==None: # assume there are no paddings in the input
			attention_mask = torch.ones(input_ids.shape).to(self.bert.device) # (bsz,seq_len)
		attention_mask = torch.cat((prefix_attention_mask, attention_mask), dim=1) # (bsz,prefix_len+seq_len)
