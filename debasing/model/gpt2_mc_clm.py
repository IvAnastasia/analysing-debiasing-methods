...
class GPT2MultipleChoiceLMHeadModel(GPT2PreTrainedModel):
	...
	def forward(
		...
		transformer_outputs = self.transformer(
			input_ids, # (bsz*n_choices,seq_len)
			past_key_values=past_key_values,
			attention_mask=attention_mask, # (bsz*n_choices,seq_len)
			token_type_ids=token_type_ids,
			position_ids=position_ids,
			head_mask=head_mask,
...
class GPT2PrefixTuningMultipleChoiceLMHeadModel(GPT2PreTrainedModel):
	_keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]
	...
	def forward(
		...
		prefix_attention_mask = torch.ones(batch_size,self.pre_seq_len).to(self.transformer.device) # (bsz*n_choices,pre_seq_len)
		if attention_mask==None: # assume there are no paddings in the input
			attention_mask = torch.ones(input_ids.shape).to(self.transformer.device) # (bsz*n_choices,seq_len)
...
class GPT2PromptTuningMultipleChoiceLMHeadModel(GPT2PreTrainedModel):
	_keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]
...
class GPT2PromptingMultipleChoiceLMHeadModel(GPT2PreTrainedModel):
	'''
